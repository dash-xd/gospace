...
type Options struct { MaxRequestBody int64; MaxResponseBody uint32 }
type EngineOptions struct { MemoryLimitPages uint32 }
type Engine struct { runtime wazero.Runtime }

func NewEngine(ctx context.Context, options EngineOptions) (*Engine, error) {
	if options.MemoryLimitPages == 0 { options.MemoryLimitPages = defaultMemoryLimitPages }
	runtimeConfig := wazero.NewRuntimeConfig().WithCloseOnContextDone(true).WithMemoryLimitPages(options.MemoryLimitPages)
	runtime := wazero.NewRuntimeWithConfig(ctx, runtimeConfig)
	if _, err := wasi_snapshot_preview1.Instantiate(ctx, runtime); err != nil { _ = runtime.Close(ctx); return nil, fmt.Errorf("instantiate WASI: %w", err) }
	return &Engine{runtime: runtime}, nil
}
func (e *Engine) Close(ctx context.Context) error { if e == nil || e.runtime == nil { return nil }; return e.runtime.Close(ctx) }
func (e *Engine) Compile(ctx context.Context, wasmBytes []byte, options Options) (*Handler, error) {
	...
	compiled, err := e.runtime.CompileModule(ctx, wasmBytes); if err != nil { return nil, fmt.Errorf("compile WASM router: %w", err) }
	return &Handler{engine:e, compiled:compiled, maxRequestBody:options.MaxRequestBody, maxResponseBody:options.MaxResponseBody}, nil
}

type Handler struct {
	engine *Engine; compiled wazero.CompiledModule; maxRequestBody int64; maxResponseBody uint32; ownsEngine bool
	mu sync.Mutex; cond *sync.Cond; active int; closing bool
}
func NewHandler(ctx context.Context, wasmBytes []byte, options Options) (*Handler, error) {
	engine, err := NewEngine(ctx, EngineOptions{}); if err != nil { return nil, err }
	h, err := engine.Compile(ctx, wasmBytes, options); if err != nil { _ = engine.Close(ctx); return nil, err }; h.ownsEngine=true; return h,nil
}
func (h *Handler) begin() bool { h.mu.Lock(); defer h.mu.Unlock(); if h.cond==nil { h.cond=sync.NewCond(&h.mu) }; if h.closing{return false}; h.active++; return true }
...
func readBody(body io.ReadCloser,limit int64)([]byte,error){if body==nil{return nil,nil};data,err:=io.ReadAll(io.LimitReader(body,limit+1));if err!=nil{return nil,fmt.Errorf("read request body: %w",err)};if int64(len(data))>limit{return nil,fmt.Errorf("request body exceeds %d bytes",limit)};return data,nil}
